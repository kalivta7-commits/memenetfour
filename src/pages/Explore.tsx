...
function TokenSkeleton() {
  return (
    <div className="explore-card animate-pulse">
      <div className="w-8 h-5 rounded bg-white/5 flex-shrink-0" />
      <div className="w-11 h-11 rounded-full bg-white/5 flex-shrink-0" />
      <div className="flex-1 min-w-0 space-y-2">
        <div className="h-3.5 bg-white/5 rounded w-24 max-w-full" />
        <div className="h-3 bg-white/5 rounded w-16 max-w-full" />
      </div>
      <div className="h-4 bg-white/5 rounded w-12 flex-shrink-0" />
    </div>
  );
}
    ...
    <div className="explore-post-row animate-pulse">
      ...
      <div className="flex-1 min-w-0 space-y-2">
        <div className="flex gap-2 min-w-0">
          <div className="h-3.5 bg-white/5 rounded w-24 max-w-full" />
          <div className="h-3.5 bg-white/5 rounded w-12 max-w-full" />
        </div>
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const enriched = (events ?? []).map((ev: any) => ({
        ...ev,
        signal_type: ev.data?.signal_type ?? ev.type ?? null,
        media_url:   ev.data?.media_url ?? null,
        thread_id:   ev.data?.thread_id ?? null,
        parent_id:   ev.data?.parent_id ?? null,
        is_reply:    ev.data?.is_reply ?? false,
        source_info: ev.data?.source ?? null,
    ...
    <div className="explore-root">
      ...
      <div className="explore-page-header">
        <div className="min-w-0 flex-1">
          <h1 className="explore-title truncate">
            Explore
            <span className="explore-title-accent">Hub</span>
          </h1>
          <p className="explore-subtitle truncate">Token signal dashboard · AI-powered intelligence</p>
        </div>
        <button
          ...
          <div className="explore-token-list">
            {loadingTokens
              ...
              : trending.map((token, idx) => (
                ...
                >
                  <span className="explore-rank flex-shrink-0">#{idx + 1}</span>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1 flex-wrap min-w-0">
                      <span className="text-white font-bold text-sm uppercase truncate group-hover:text-[#00FF88] transition-colors font-['Syne'] max-w-[120px] xs:max-w-[200px]">
                        {post.token_name}
                      </span>
                      <span className="text-white/35 font-mono text-xs truncate max-w-[80px]">
                        ${post.token_ticker}
                      </span>
